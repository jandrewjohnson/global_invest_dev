...
#     # Convert crop_filenm to lowercase
#     crop_filenm_lower = crop_filenm.lower()
...
#         # Find the best match for crop_filenm in price_df['Item'] using fuzzy matching
#         best_match = process.extractOne(crop_filenm_lower, price_df['Item_lower'], score_cutoff=80)

#         if best_match:
#             matched_item = best_match[0]
#             print(f"Best match for {crop_filenm}: {matched_item}")
...
#         else:
#             print(f"No good match found for {crop_filenm} in price data.")
#             best_matches.append({
#                 'crop_filenm': crop_filenm,
#                 'matched_item': 'No match found',
#                 'production_raster': crop_production_raster,
...
#             # Store the total value for the crop in the list for later saving to CSV
#             total_values.append({'crop_filenm': crop_filenm, 'overall_total_value': overall_total_value})
...
#             # Store the best match in the list for later saving to CSV
#             best_matches.append({'crop_filenm': crop_filenm, 'matched_item': matched_item})
...
#         else:
#             print(f"No good match found for {crop_filenm} in price data.")
#             best_matches.append({'crop_filenm': crop_filenm, 'matched_item': 'No match found'})
...
#             # Filter the price data using the best match
#             crop_price_df = price_df[price_df['Item_lower'] == matched_item]
...
#         # Filter the price data for the current crop and "Producer Price (USD/tonne)"
#         crop_price_df = price_df[(price_df['Item'] == crop_filenm) & (price_df['Element'] == 'Producer Price (USD/tonne)')]
#         print(crop_price_df)
...
#         production_df = output_table
#         # # Filter the price data for "Apples" and "Producer Price (USD/tonne)" in the year 2000
#         apple_price_df = price_df[(price_df['Item'] == 'Apples') & (price_df['Element'] == 'Producer Price (USD/tonne)')]
...
#         # Save the merged DataFrame to a new CSV file with the total value column included
#         output_path = r"D:\Shared drives\NatCapTEEMs\Projects\Global GEP\Ecosystem Services SubFolders\Pollination\crop_production_by_country\pollination_production_with_price_data_new.csv"
#         merged_df.to_csv(output_path, index=False)
...
# production_data_path = r"D:\Shared drives\NatCapTEEMs\Projects\Global GEP\Ecosystem Services SubFolders\Pollination\crop_production_by_country\pollination_dependent_apple_production_crop_production_by_country.csv"
# price_data_path = r"D:\Shared drives\NatCapTEEMs\Projects\Global GEP\Ecosystem Services SubFolders\Pollination\Price_data\Prices_E_All_Data_ori.csv"
